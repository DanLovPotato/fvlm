"""清理 organ_annotation.json 里 `_indices` 列表中的 null 占位符（validation 版）。

跟 clean_null_indices.py 的唯一区别：那个脚本处理 find_similar_reports.py 输出的
train 自检索结果，顶层 key 是 "train"；这个脚本处理 find_similar_reports_val.py
输出的结果，顶层 key 是 "validation"。其余逻辑（为什么会有 null、为什么要清理、
清理规则）完全一样，见 clean_null_indices.py 的模块docstring。

背景：find_similar_reports_val.py 生成 `<organ>_indices` 时，如果某个 (validation
病人, 器官) 没有有效的 image embedding（这个器官的 mask 体素数为 0，见
find_similar_reports.py 的 compute_image_embeddings 里的
`if whole_organ_sizes[organ] == 0: continue`），会把这一整行 TOP_K 个位置都填成 null
（find_similar_reports_val.py main() 里 `... if idx >= 0 else None`）。
...
用法：
    python clean_null_indices_val.py                 # 清理默认路径，覆盖原文件
    python clean_null_indices_val.py --dry-run        # 只打印统计，不写文件
    python clean_null_indices_val.py --input PATH --output PATH   # 自定义路径
"""
import argparse
import json
import os

DEFAULT_PATH = "/mnt/researchdrive/ptiwari9/Staff_Trainee_Folders/Dan/chestCT/data/dataset/EK_files_val/organ_annotation.json"


def clean_null_indices(data):
    """原地过滤 data['validation'] 里每条记录的 `<organ>_indices` 字段，返回统计信息。"""
    total_lists = 0
